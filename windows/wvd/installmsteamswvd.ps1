...
mkdir "C:\temp\msteams" -Force
Start-Transcript -Path "C:\temp\msteams\install\ps_log.txt" -Append
Write-Host "######################################
...
Start-Process C:\Windows\System32\msiexec.exe -ArgumentList  '/i C:\Windows\Temp\msteams_sa\install\Teams_windows_x64.msi /l*v C:\temp\msteams\teams_install_log.txt ALLUSER=1 ALLUSERS=1 /qn /norestart' -wait
...
Start-Process C:\Windows\System32\msiexec.exe -ArgumentList '/i C:\Windows\Temp\msteams_sa\install\MsRdcWebRTCSvc_x64.msi /l*v C:\temp\msteams\WebRTC_install_log.txt /qn /norestart' -Wait

write-host "Finished running installers. Check C:\temp\msteams_sa for logs on the MSI installations."
